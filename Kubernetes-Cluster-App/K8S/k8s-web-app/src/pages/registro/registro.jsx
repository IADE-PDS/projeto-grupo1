...
function Registro(){
    ...
    const enviarDadosParaBackend = async () => {
      ...
      try {
        ...
        const user = await axios.post('http://192.168.64.6:31332/api/users', {
          email: email,
          username: name,
          password: password,
          location: gpslocation,
        ...
        */
        ...
        const token = await axios.post('http://192.168.64.6:31332/api/users/auth',{
          username: name,
          password: password
        })

        // Recebe os dados da base de dados client creado e ja aunticado.
        const u = await axios.get(`http://192.168.64.6:31332/api/users/${name}`)
        navigate('/')
